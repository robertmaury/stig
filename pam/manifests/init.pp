...
class pam {
  ...
  # sshd includes password-auth, not system-auth, one possible 
  # solution to enable pam_lastlog.so for SSH follows
  #
  ...
  exec {
    'Set Password Hashing Algorithm in /etc/libuser.conf':
      command => "/bin/sed -i 's/crypt_style[ \t]*=[ \t]*md5/crypt_style = sha512/g' /etc/libuser.conf",
            unless  => '/bin/grep crypt_style /etc/libuser.conf | grep sha512';
  }
}
